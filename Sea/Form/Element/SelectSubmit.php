...
require_once 'Sea/Form/Element/Select.php';
require_once 'Sea/Form/Element/Submit.php';


class Sea_Form_Element_SelectSubmit extends Sea_Form_Element_Container {
	 ...
	 */
	public function __construct($select, $submit, $label = '', $options = array()) {
		
		// traitement des arguments
		$select = $select instanceof Sea_Form_Element_Select ? $select : new Sea_Form_Element_Select($select);
		$submit = $submit instanceof Sea_Form_Element_Submit ? $submit : new Sea_Form_Element_Submit($submit);
		
		// constrcutruction de l'objet
		parent::__construct($select->getName().$this->_suffix, $options);
